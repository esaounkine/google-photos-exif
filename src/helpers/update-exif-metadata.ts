...
export async function updateExifMetadata(fileInfo: MediaFileInfo, outputFilePath: string, timeTaken: string, errorDir: string): Promise<void> {
  if (!doesFileSupportExif(outputFilePath)) {
    return;
  }
  ...
  try {
    ...
    await copyFile(outputFilePath,  resolve(errorDir, fileInfo.mediaFileName));
